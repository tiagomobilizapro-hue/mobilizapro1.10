...
## 1.10.2 - Enterprise Core / MySQL First
- Núcleo backend reforçado em `api/bootstrap.php`.
- Transações centralizadas via `mobi_transaction()`.
- Log estruturado em `storage/mobilizapro-error.log`.
- Auditoria operacional centralizada com IP e user-agent.
- `api/store.php` usando transação centralizada.
- Criado `api/health.php` para diagnóstico administrativo.
- Estética atual preservada.
